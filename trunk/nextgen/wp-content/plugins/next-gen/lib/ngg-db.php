...
class nggdb {
    ...
    */
     ...
     * @return array $galleries
     */
    function find_all_user_galleries($order_by = 'gid', $order_dir = 'ASC', $counter = false, $limit = 0, $start = 0, $exclude = true, $userID) {      
        global $wpdb; 
        ...
        {
        	$where_clause = ' WHERE author = ' . $userID . ' ';
        } 
        $this->galleries = $wpdb->get_results( "SELECT SQL_CALC_FOUND_ROWS * FROM $wpdb->nggallery $where_clause ORDER BY {$order_by} {$order_dir} {$limit_by}", OBJECT_K );
